'''version3 seed words algorithm for notalgic IR corpus 05/11'''
'''changed the PMI calculation strategy use P(x,y)/P(x)=P(y|x) instead of P(x|y)/P(x) ----
...
file1 = open('D:/uw course/capstone/mypersonality/ECtest3_punctation_len3_freqover0.txt')#candidate
...
for p in range(0,canl):
    for q in range(0,keyl):
        if candidatecount[p]*keywordcount[q]*cooccur[p][q]>0:
            ...
            PMI[p][q]=cooccur[p][q]/(300*candidatecount[p])
        else:
            PMI[p][q]=0
print(PMI)
#print(keywordcount)
'''--------------------------'''
file3 = open('D:/uw course/capstone/mypersonality/wordlist_1gram_freqover0_IR2_21w_thre0.01_nos.txt','w')
Threshold = 0.01
keywordlist1 = []
for p in range(0,canl):
    PMI_MAX = PMI[p][0]
...
file4 = open('D:/uw course/capstone/mypersonality/wordlist_1gram_freqover0_IR2_21w_thre0.05_nos.txt','w')
Threshold = 0.05
keywordlist2 = []
for p in range(0,canl):
    PMI_MAX = PMI[p][0]
...
file5 = open('D:/uw course/capstone/mypersonality/wordlist_1gram_freqover0_IR2_21w_thre0.001_nos.txt','w')
Threshold = 0.001
keywordlist3 = []
for p in range(0,canl):
    PMI_MAX = PMI[p][0]
...
file6 = open('D:/uw course/capstone/mypersonality/wordlist_1gram_freqover0_IR2_21w_thre0.1_nos.txt','w')
Threshold = 0.1
keywordlist4 = []
for p in range(0,canl):
    PMI_MAX = PMI[p][0]
